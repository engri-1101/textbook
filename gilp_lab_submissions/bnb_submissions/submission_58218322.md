...
```
    ...
    NoInteriorPoint                           Traceback (most recent call last)

    /usr/local/lib/python3.6/dist-packages/gilp/_geometry.py in polytope_vertices(A, b, interior_pt)
         86         if interior_pt is None:
    ---> 87             interior_pt = interior_point(A,b)
         88         interior_pt = interior_pt.astype(float)


    /usr/local/lib/python3.6/dist-packages/gilp/_geometry.py in interior_point(A, b, tol)
        198     if x[-1] <= tol:
    --> 199         raise NoInteriorPoint('Halfspace intersection has no interior point.')
        200     return x[:-1]
    ...
    <ipython-input-52-c5d79fee6538> in <module>()
    ----> 1 nodes = gilp.bnb_visual(gilp.examples.STANDARD_2D_IP)
    

    /usr/local/lib/python3.6/dist-packages/gilp/visualize.py in bnb_visual(lp, manual, feas_tol, int_feas_tol)
    ...
    /usr/local/lib/python3.6/dist-packages/gilp/visualize.py in labeled_feasible_region(lp, theme, basic_sol, show_basis, vertices)
        280     """
        281     if vertices is None:
    --> 282         vertices = lp.get_vertices()
        283     region = feasible_region(lp=lp,
        284                              theme=theme,


    /usr/local/lib/python3.6/dist-packages/gilp/simplex.py in get_vertices(self)
        259         A_tmp = np.vstack((A, -np.identity(n)))
        260         b_tmp = np.vstack((b, np.zeros((n,1))))
    --> 261         return polytope_vertices(A_tmp, b_tmp)
        262 
        263 
    ...
    /usr/local/lib/python3.6/dist-packages/gilp/_geometry.py in polytope_vertices(A, b, interior_pt)
        101                 pass
        102         vertices = np.round(np.array(vertices), 12)
    --> 103         vertices = np.unique(vertices, axis=0)
        104     return [np.array([v]).transpose() for v in vertices]
        105 
...
```
    ...
    <ipython-input-50-56b8760f18ba> in <module>()
    ----> 1 nodes[0].show()
...
```
    ...
    <ipython-input-48-51baf5263716> in <module>()
    ----> 1 nodes[1].show()
