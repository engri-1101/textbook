...
def plotNetwork(nodes, links, title='Plot of Graph', targets=None, on_map=False):
    ...
    :param path: path represented by list of arcs
...
def plotShortestPathTree(dfn, dfl, path):
    
    in_tree = pd.Series(np.zeros(dfn.shape[0]))
    in_tree[list(zip(*path))[0]] = 1
    in_tree = pd.Series(np.ones(dfl.shape[0])).index.map(in_tree)
